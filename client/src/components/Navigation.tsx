...
export function BottomNav() {
  ...
  return (
    <nav className="w-full bg-[#0f172a] h-20 border-t border-white/10 flex items-center justify-around px-4">
      {navItems.map((item) => (
        <button 
          key={item.id}
          onClick={() => setLocation(item.id)} 
          className={`flex flex-col items-center flex-1 py-3 transition-all duration-300 ${location === item.id || (item.id === "/" && location.startsWith("/category")) ? 'text-[#f97316]' : 'text-white/40'}`}
        >
...
export function AdBanner({ type }: { type: 'hero' | 'footer' | 'content' }) {
  const styles = {
    ...
    footer: "w-full h-[60px] bg-white border-t border-gray-100 flex items-center justify-center",
